...
export const ScenarioCard: React.FC<ScenarioCardProps> = ({ scenario, onEdit, onDelete, t }) => {
    const title = scenario.topic || scenario.title || 'Untitled Scenario';
    
    const getDisciplineLabel = (d: string) => {
        const safeKey = d.toLowerCase().replace(/ /g, '_');
        const key = `discipline_${safeKey}`;
        const translated = t(key);
        return translated && !translated.startsWith('discipline_') ? translated : d;
    };

    const category = scenario.disciplines?.map(d => getDisciplineLabel(d)).join(', ') || scenario.category || 'General';
    const description = scenario.tasks?.[0]?.description || scenario.opening?.description || 'No description available.';
    const primaryImage = scenario.tasks?.[0]?.coverImage || scenario.opening?.imageUrl;
    const imageUrl = primaryImage && primaryImage.trim().length > 0 ? primaryImage : placeholderImage;
    const duration = typeof scenario.duration === 'number' ? `${scenario.duration} ${t('min') || 'min'}` : scenario.duration;
        ...
        <div className="group bg-white p-4 rounded-2xl border border-surface-200 shadow-sm hover:shadow-lg hover:border-primary-200 transition-all duration-300 flex flex-col md:flex-row gap-6 items-center">
            ...
            <div className="flex-1 text-center md:text-start rtl:md:text-right w-full">
                <div className="flex flex-col md:flex-row gap-2 items-center md:items-start rtl:md:items-start mb-2">
                    <h3 className="text-xl font-bold text-surface-900 group-hover:text-primary-600 transition-colors">{title}</h3>
                    <span className="px-2.5 py-0.5 bg-primary-50 text-primary-700 text-xs font-bold rounded-full border border-primary-100 uppercase tracking-wide">{category}</span>
                </div>
                <p className="text-surface-500 text-sm line-clamp-2 mb-3">{description}</p>
