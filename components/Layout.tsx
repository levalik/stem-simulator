...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className={`min-h-screen bg-zinc-50 flex flex-col font-sans ${language === 'he' ? 'rtl' : 'ltr'}`}>
      <header className="fixed w-full top-0 z-50 glass border-b border-zinc-200/60 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer group">
            <div className="w-9 h-9 bg-gradient-primary rounded-xl shadow-lg shadow-violet-500/20 flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
              <Logo className="w-6 h-6" classNamePath="fill-white" />
            </div>
            <span className="text-xl font-bold text-gradient tracking-tight">STEM Simulator</span>
          </div>
      ...
      <footer className="bg-white border-t border-zinc-100 py-8 mt-auto">
        <div className="max-w-7xl mx-auto px-4 flex flex-col items-center justify-center text-center space-y-2">
          ...
          <p className="text-xs text-zinc-300">{t('powered_by')} Google Gemini</p>
        </div>
      </footer>
    </div>
  );
